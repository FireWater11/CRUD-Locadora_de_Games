...
export async function listar_jogos_id(req, res) {
    const id_listar = parseInt(req.params.id);
...
export async function atualizar_jogo(req, res) {
    ...
    try {
        ...
        const { title, plataform, available } = req.body;

        if (!title || !plataform || available === undefined) {
            return res.status(404).json({ mensagem: "Necessita de title, plataform e available na requisição." });
        };

        const id_game_update = await prisma.games.update({
            where: {
                id:Number(id_update)
            },
        ...
        res.status(200).json({ mensagem: "Jogo atualizado: ", id_game_update });
